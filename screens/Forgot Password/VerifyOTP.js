...
const ForgotPassword = ({pin,setPin,recoveryData, setRecoveryData, navigation}) => {
	...
	const verifyButton = () =>{
		if(!pin)return ToastFunction("error", "Invalid pin kindly click the resend button");
		...
		if(enteredPin.first !==pins[0]  || enteredPin.second !==pins[1] || enteredPin.third !== pins[2] && enteredPin.fourth!==pins[3]){
			return ToastFunction("error", "Incorrect pin kindly check your pin");
		}
		...
		<SafeAreaView style={{ ...styles.container, backgroundColor: 'white' }}>
			<View style={styles.containerWhite}>
				...
				<View style={{ flexDirection: 'row', width: '80%', gap: 10 }}>
					...
					<TextInput value={enteredPin.second} onChangeText={(text)=>handleOnChange("second", text)} keyboardType={"phone-pad"} value={enteredPin.second} style={{ padding: 10, fontSize: 20, borderBottomColor: '#06b6d4', borderBottomWidth: 2, flexGrow: 1, textAlign: 'center' }} maxLength={1} />

					<TextInput value={enteredPin.third} onChangeText={(text)=>handleOnChange("third", text)} keyboardType={"phone-pad"} value={enteredPin.third} style={{ padding: 10, fontSize: 20, borderBottomColor: '#06b6d4', borderBottomWidth: 2, flexGrow: 1, textAlign: 'center' }} maxLength={1} />

					<TextInput value={enteredPin.fourth} onChangeText={(text)=>handleOnChange("fourth", text)} keyboardType={"phone-pad"} value={enteredPin.fourth} style={{ padding: 10, fontSize: 20, borderBottomColor: '#06b6d4', borderBottomWidth: 2, flexGrow: 1, textAlign: 'center' }} maxLength={1} />
				</View>

				{
					timer > 0 
					? <Text>Try to resend data after <Text style={{color:"red"}}>{timer}</Text> second</Text>
					: <Text>Didn't receive any pin? <Text style={{color:"red"}} onPress={resendButton}>Resend</Text></Text>
				}
